# ------------------------------------------------------------------------
# rect11_lcd.py
# 基于 rect11.py 的 LCD 显示版本：
# 1. 识别流程、16:9 比例 (320x180)、LAB 阈值、动态 ROI 追踪与形态学参数与 rect11.py 一致。
# 2. 主要差异：显示后端从 IDE 虚拟显示改为 LCD 屏幕 (ST7701)，关闭 to_ide 镜像输出。
# 3. 显示时复用一张 800x480 RGB565 LCD 帧，通过 draw_image 将 320x180 画面拉伸到 LCD 全屏。
# 4. 算法修正：寻找摄像头镜头中心与矩形中心偏上 1.5 cm 的标定目标点，计算实际 Yaw/Pitch。
# 5. 计算并实时在屏幕上以及控制台输出目标点实际角度与理想状态 (Yaw 0°, Pitch 3°) 的偏差。
# 6. 输出格式与变量命名（dx, dy, Yaw, Pitch）与 rect10_lcd 保持完全一致，方便上位机无缝解析。
# ------------------------------------------------------------------------
...
# --- MSPM0 UART 通信配置 ---
# 与 k230/uart1_comm_test.py 保持一致：K230 引脚 11/12 复用到 UART2，
# 对应当前小车接线中标注的 K230 UART1 通信口。
K230_UART_TX_PIN = 11
K230_UART_RX_PIN = 12
...
LASER_ENABLE_PITCH_ERROR_DEG = 1.5
LASER_ENABLE_CONFIRM_FRAMES = 3
...
def is_laser_error_small(yaw_err, pitch_err):
    return (abs(float(yaw_err)) <= LASER_ENABLE_YAW_ERROR_DEG and
            abs(float(pitch_err)) <= LASER_ENABLE_PITCH_ERROR_DEG)
...
def capture_picture():
    ...
    last_tx = 0               # 偏上目标点 x
    last_ty = 0               # 偏上目标点 y
    last_yaw_err = 0.0        # 目标点 Yaw 偏差
    last_pitch_err = 0.0      # 目标点 Pitch 偏差
    display_yaw_err = 0.0     # 顶部状态栏显示用 Yaw 偏差
    display_pitch_err = 0.0   # 顶部状态栏显示用 Pitch 偏差
    display_err_valid = False # 顶部状态栏角度误差是否有效
    ...
    while True:
        ...
        try:
            ...
            if best_rect is not None:
                ...
                # 借助靶标物理高度 17.3cm 与其图像像素高度的比例，投影计算偏上 1.5cm 点的像素坐标
                r_h = best_rect.h()
                pixel_per_cm = r_h / TARGET_HEIGHT_CM
                offset_y_pixels = CALIB_OFFSET_CM * pixel_per_cm

                target_x = t_cx
                target_y = int(t_cy - offset_y_pixels)

                # 控制和通信都使用 rect11 标定后的目标点，而不是矩形几何中心。
                # MCU 侧会把该目标点收敛到由 IDEAL_YAW/PITCH 反算出的标定参考点。
                dx = target_x - IMG_CENTER_X
                dy = target_y - IMG_CENTER_Y
                ...
                # 绘制 1.5cm 偏上目标点 (用实心小圆圈表示)
                img.draw_circle(target_x, target_y, 4, color=255, fill=True)

                # 绘制当前正在生效的局部 ROI 边框
                img.draw_rectangle(search_roi, color=255, thickness=1)

                # 辅助参考线和文本 (连接摄像头中心与标定目标点)
                ...
                display_yaw_err, display_pitch_err = yaw_err, pitch_err
                ...
                if tracking_state == STATE_COASTING:
                    ...
                    img.draw_circle(last_tx, last_ty, 4, color=255, fill=True)
                    img.draw_rectangle(search_roi, color=255, thickness=1)
...
def main():
    ...
    try:
        print("--- rect11_lcd 启动 (Sensor id=2 + 16:9 LAB 标定目标点 LCD 版) ---")
        camera_init()
        camera_is_init = True
