...
- 底层为**阻塞 I2C**，与同总线的 `bsp/mpu6050` 同构：写 1 字节命令（抑制 STOP）→ REPEATED
  START → 读 N 字节；软件重启为纯写 1 字节命令。
...
- 读命令内部对瞬态总线错误重试 3 次（`GANV_GRAY_RETRY`）；若仍频繁失败（online/offline
  交替），优先检查 I2C 总线上拉——三从机共总线时手册建议外置约 10K 上拉到 3.3V。
