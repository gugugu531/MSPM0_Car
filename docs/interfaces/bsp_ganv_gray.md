...
- 底层为**阻塞 I2C**，采用手册方法 2/3 的「命令与读取分离」：写 1 字节命令（独立事务，带 STOP）
  + 独立读 N 字节（START+RX+STOP）；软件重启为纯写 1 字节命令。早期曾用 repeated-start（TX
  抑制 STOP 后靠 BUSY 捕捉中间态），因时序竞争导致读取隔次交替失败，已改为独立事务根治。
...
- 读命令内部对瞬态总线错误重试 `GANV_GRAY_RETRY` 次作兜底；确定性的隔次交替已由独立事务
  根治，此重试仅用于真·瞬态（NACK / 上升沿不足 / 传感器忙）。若仍频繁失败，检查 I2C 总线
  上拉——三从机共总线时手册建议外置约 10K 上拉到 3.3V。
