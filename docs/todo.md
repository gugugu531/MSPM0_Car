...
> 本节记录 2026-07-26 代码审查发现的长期维护风险。除已经由代码直接证明的结构事实外，
> 对运行效果的影响仍须通过上板遥测确认。当前阶段只登记，不在其他功能开发中顺手扩改。
...
| P2 | OLED 行格式化没有容量参数 | `PutStr`、`Straight_PutStr`、`AppFmt_*` 依靠调用者预估数组长度 | 先提供有容量的 app/UI 行构建器，再逐页迁移；不得让 BSP 依赖 UI 格式化 |
| P2 | `[STR]` 字段和模式编号与 Python 工具分别硬编码 | C 枚举与 `MODE_NAMES/FLOAT_FIELDS` 无版本协商 | 仅为直行遥测增加稳定编号和协议版本；不改蓝牙或其他调试协议 |
| P2 | `app_checks.c` 已聚合多类设备诊断 | 姿态、灰度、电机、编码器和 PID 检查集中在同一文件 | 需要新增检查任务时优先按子系统拆文件，保持任务描述符接口不变 |
| P1 | 缺少主机侧纯计算回归测试 | 当前构建只能验证编译，不能覆盖角度跨界、500 ms 切换、丢帧恢复和混控极值 | 测试只链接纯计算/可注入依赖的模块，不为测试把硬件依赖带入 core |
...
4. **修复按边界分步提交**：JY61P 快照、直行状态机、距离语义、安全格式化、遥测版本和
   检查任务拆分分别提交；不得以一次“大整理”跨层修改所有模块。
5. **先建立回归再迁移**：状态机或公式调整前，至少覆盖角度跨越 `±180°`、A/B 切换连续性、
   IMU 丢帧恢复、正负满量程混控和停车距离语义；编译通过不能替代这些行为验证。
