...
S4(D→A)` 单向状态机；边界里程已扣除灰度阵列距车尾的 `0.195 m`，使前置阵列到达拐点时
开始建立/撤销弯道差速。`[TRK] seg` 输出当前段，`vs` 输出半差速指令。直线段无曲率前馈，
右半圆段叠加差速轮速前馈（S2/S4 比例分别为 `1.10/1.00`），灰度与陀螺闭环在四段内持续工作。
S4 末段用 `FOLLOW→XLINE→OFFSET→STOP` 完成 A 横线进出沿取中心、车尾 `195 mm` 几何补偿
和最后 `5 mm` 电子制动；`[TRK] run/fs` 分别标识试跑编号和 `LINE/ODOM/END` 停车来源。
H4 在 B 后末端纵线消失时进入 `OFFSET` 终点盲行，不按普通丢线处理。
首次成功捕线后若出现全白或灰度超时，任务锁存 `LOST` 并刹车；重新捕到黑线不会自动启动，
只能按 BACK 退出后重新进入任务。唯一例外是 S2/S4 的有效灰度瞬时全白：曲率前馈＋陀螺
最多保持 `200 ms` 尝试重捕，`[TRK] gm=1` 明确标记该有界降级；超时仍进入 `LOST`。
