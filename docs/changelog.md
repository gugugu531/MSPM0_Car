...
- **按 H 题要求 2～6 重构循迹任务并加入四段赛道状态机与 Debug_Ex 上位机**：
  ...
  - H2 使用 `0.45 m/s` 空载配置；H4/H5/H6 使用 `0.26 m/s`、`0.12 m/s²` 载球安全配置。
    ...
    左右轮平均目标速度保持不变。
  - H2/H5/H6 实现 `S1→S2→S3→S4` 单向里程状态机。按车尾起点到轮轴 `70 mm` 偏置，
    临时边界为 `1.4300/3.0008/4.5008 m`；S2/S4 按 `L_eff=0.15 m`、`r=0.5 m` 叠加
    右弯差速前馈，变化率限制形成约 `0.4 s` 入/出弯过渡。参数仍待 Track Teach 实测覆盖。
  ...
  - CMSIS/ARMCLANG 6.22 全工程构建通过，上位机 5 项单元测试、绘图 smoke、文档与工程同步
    检查通过；**尚未上板验证**。
