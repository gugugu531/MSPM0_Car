...
- 优化 E1 `Line lost` 判定：巡线控制仍按掩码忽略中间 3、4 两路，但丢线保护改用真实 8 路灰度计数，少于 2 路压线并持续超过宽限时间才判定丢线。
- 巡线控制改为通过 `LINE_TRACKING_ACTIVE_SENSOR_MASK = 0xE7` 忽略中间 3、4 两路灰度传感器，只使用最左侧 3 路和最右侧 3 路；E1 转弯识别和恢复循线判断同步使用该掩码。
