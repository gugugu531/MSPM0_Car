...
> 决定、固件只能跟随**，管运动平顺度；后者 = 停下时离目标多远（到位容差 1 计数
> = ±0.18°，**已压到编码器分辨率这个物理地板**），管落点准不准。
> **最小可指令位移由到位容差决定，不由细分数决定**——一个微步远小于容差，那样的目标
> 伺服直接判为已到位、一个脉冲都不发；`JOG` 最细一档因此取 `2×容差+1` = 3 计数 = 0.54°
> （`2×` 是因为残余误差与行进方向同号，换向会抵掉一份，否则换向第一下必然死键）。
> ⚠ 改细分必须**先拨拨码再改宏**，两边不一致不报错，只让转速整体差固定倍数。
> 到位判定**带回差**：进门看容差（1）、出门看 `SERVO_RESUME_COUNTS`（6），使"停得准"与
> "漂多少才重新动"解耦（后者挡机械下沉，容差收紧时刻意不跟着收）；`IsAtTarget()` 直接
> 返回该状态。完整分析见标定手册「精细度：两件独立的事」。
