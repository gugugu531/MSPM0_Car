 ...
 *
 * 公式仍写成 `2 × 容差 + 1`，因为它同时覆盖两个约束，容差非 0 时缺一不可：
 *
 *   伺服是从带外接近、一进带就停，所以停下来时残余误差的量值≈容差，且**符号与刚才
 *   的行进方向相同**（目标在前、实测在后）。正向点完停在 err = +容差；这时按反向，
 *   新误差 = +容差 − 步长，量值只有 `步长 − 容差`。要它还在容差带外就必须
 *   `步长 − 容差 > 容差`，即 **步长 > 2 × 容差**。取「容差 + 1」的话换向后 |err| 恰好
 *   等于容差，伺服判为已到位——**换向第一下必然是死键**，确定性发生，不是偶发。
 *
 *   容差现已取 0，残余误差恒为 0，换向不再被抵掉一份——公式退化成 1 个计数，
 *   **换向的 2 倍代价随容差一起归零了**。留着 `2×` 是为了容差以后抬回去时自动正确。
 *
 * 中/粗两档**写绝对计数，不再按细档取倍数**：容差已压到 0，细档 = 1 个计数，任何倍数
 * 都会把中/粗两档一起拖小（原来的 8 倍/25 倍会变成 8 和 25 计数，粗档横跨行程要按
 * 十六下）。这两档要的是绝对手感，与容差无关：软限位内可用行程 SOFT_MAX - SOFT_MIN
 * ≈ 410 计数，粗档 75 计数意味着五六下能从一端走到另一端，中档 20 计数是「看得见但
 * 不过头」的调整量。
 *
...
#define SM_JOG_STEP_MID    20
#define SM_JOG_STEP_COARSE 75

/* 三档必须严格递增，且都得跨过换向阈值，否则某一档会出现「换向第一下死键」。 */
#if (SM_JOG_STEP_MID <= SM_JOG_STEP_FINE) || (SM_JOG_STEP_COARSE <= SM_JOG_STEP_MID)
#error "SM_JOG_STEP_FINE/MID/COARSE 必须严格递增"
#endif
