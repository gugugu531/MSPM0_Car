 ...
 * JOG 的三档步长，单位编码器计数。
 *
 * 最细一档不是「一个微步」：一个微步是 STEP_ANGLE_DEG / MICROSTEP = 0.225°，换算过来
 * 只有 1.25 个编码器计数，小于伺服的到位容差 STEP_MOTOR_POSITION_TOLERANCE_COUNTS（2）
 * ——那样的目标伺服直接判为已到位，一个脉冲都不会发。**闭环下的「最小可指令位移」由
 * 到位容差决定，不是由细分数决定**。
 *
