/**
 * @file  oled.c
 * @brief SSD1306 OLED 硬件 I2C 显示实现 (帧缓冲模型)。
 *
 * 绘制操作只写 1KB RAM 帧缓冲 (纯 CPU, 极快); OLED_Flush() 用水平寻址一次事务
 * 把整帧刷到屏幕 (消除旧版 8 字节分包低效)。ui 层在每次渲染/更新末尾调 Flush。
 * (Step1: Flush 为阻塞单事务; Step2 将改为 DMA + 完成中断, 使传输全异步。)
 */
...
/* 帧缓冲: [0]=0x40 数据控制字 (供整帧一次事务), [1..]=像素 (page-major: page*128+col)。 */
#define OLED_FB_PIXELS (OLED_WIDTH * OLED_PAGE_COUNT)   /* 128 * 8 = 1024 */
static uint8_t oled_fb[1U + OLED_FB_PIXELS];
#define OLED_FB (oled_fb + 1U)   /* 像素区起始 */
...
/*
 * 把整帧缓冲刷到屏幕: 水平寻址窗口设为全屏, 再一次事务发送 [0x40][1024 像素]。
 * (Step1 阻塞; Step2 改 DMA。) 水平寻址下写满 128 列自动换页, 1024 字节铺满全屏。
 */
void OLED_Flush(void){
    static const uint8_t win_col[3] = {0x21U, 0x00U, 0x7FU};   /* 列范围 0..127 */
    static const uint8_t win_page[3] = {0x22U, 0x00U, 0x07U};  /* 页范围 0..7   */

    OLED_WriteCmds(win_col, 3U);
    OLED_WriteCmds(win_page, 3U);

    oled_fb[0] = 0x40U;   /* 数据控制字 */
    (void)OLED_I2C_Write(oled_fb, (uint16_t)(1U + OLED_FB_PIXELS));
}
