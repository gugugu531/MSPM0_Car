...
/*
 * 驱动器细分数,**须与驱动器拨码一致**。实测 8,与拨码相符。
 * 不一致不会报错,只会让转速整体差固定倍数。
 */
 ...
 * 下限由两件硬事实决定——电机只能停在微步位上(每微步
 * 2000/(360/1.8×8) = 1.25 计数),编码器量化 ±0.5 计数。取 2 意味着 ±2 计数的带宽里
 * 总有 3 个以上微步位可停,余量充足;取 1 就贴着微步间距了,不建议。
 *
 * 从 5 收到 2:最小可指令位移 0.9° → 0.36°(电机轴)。收紧到位带本会让摆杆自重下沉
 * 一点就反复补偿(末端来回抖),所以同时引入了下面的回差带,把"停多准"和"漂多少才
 * 重新动"拆成两件事。
 */
#define STEP_MOTOR_POSITION_TOLERANCE_COUNTS 2
