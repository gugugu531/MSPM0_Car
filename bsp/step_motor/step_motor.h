 ...
 *
 * 下限由**微步粒度**决定:电机只能停在微步位上,×32 时每微步 0.3125 计数,所以任何
 * 整数计数的目标附近总有微步位可落(最远差 0.156 计数),容差取 1 完全够得着。
 * ×8 时代每微步 1.25 计数,那时取 1 是够不着的——是 ×32 才让这一档成为可能。
 *
 * 编码器量化(±0.5 计数)已按明确要求不作为约束考虑。它是这里唯一还没被排除的物理
 * 因素:真机上若出现"停不下来、持续小幅补偿",嫌疑就在它,把本值抬回 2 即可。
 *
 * 5 → 2 → 1 的效果:
 *   到位精度         ±0.9° → ±0.36° → **±0.18°**(电机轴)
 *   最小可指令位移   1.08° → 0.54°  → **0.36°** (= 容差+1 计数)
 *   最小换向安全步长 1.98° → 0.9°   → **0.54°** (= 2×容差+1,见 app_checks.c JOG)
 *
 * 收紧到位带本会让摆杆自重下沉一点就反复补偿(末端来回抖),所以同时有下面的回差带,
 * 把"停多准"和"漂多少才重新动"拆成两件事——回差带不跟着收,抗漂移能力不受影响。
 */
#define STEP_MOTOR_POSITION_TOLERANCE_COUNTS 1
